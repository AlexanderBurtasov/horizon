...
void spi_master_init(TFT_t *dev, int16_t GPIO_MOSI, int16_t GPIO_SCLK, int16_t GPIO_CS, int16_t GPIO_DC, int16_t GPIO_RESET, int16_t GPIO_BL)
{
	...
	if ( GPIO_CS >= 0 )
	{
		...
		gpio_reset_pin( static_cast<gpio_num_t>(GPIO_CS) );
		gpio_set_direction( static_cast<gpio_num_t>(GPIO_CS), GPIO_MODE_OUTPUT );
		gpio_set_level( static_cast<gpio_num_t>(GPIO_CS), 0 );
	}

	ESP_LOGI(TAG, "GPIO_DC=%d",GPIO_DC);
	//gpio_pad_select_gpio( GPIO_DC );
	gpio_reset_pin( static_cast<gpio_num_t>(GPIO_DC) );
	gpio_set_direction( static_cast<gpio_num_t>(GPIO_DC), GPIO_MODE_OUTPUT );
	gpio_set_level( static_cast<gpio_num_t>(GPIO_DC), 0 );

	ESP_LOGI(TAG, "GPIO_RESET=%d",GPIO_RESET);
	if ( GPIO_RESET >= 0 )
	{
		//gpio_pad_select_gpio( GPIO_RESET );
		gpio_reset_pin( static_cast<gpio_num_t>(GPIO_RESET) );
		gpio_set_direction( static_cast<gpio_num_t>(GPIO_RESET), GPIO_MODE_OUTPUT );
		gpio_set_level( static_cast<gpio_num_t>(GPIO_RESET), 1 );
		delayMS(100);
		gpio_set_level( static_cast<gpio_num_t>(GPIO_RESET), 0 );
		delayMS(100);
		gpio_set_level( static_cast<gpio_num_t>(GPIO_RESET), 1 );
		delayMS(100);
	}
	...
	if ( GPIO_BL >= 0 )
	{
		...
		gpio_reset_pin(static_cast<gpio_num_t>(GPIO_BL));
		gpio_set_direction(static_cast<gpio_num_t>(GPIO_BL), GPIO_MODE_OUTPUT);
		gpio_set_level(static_cast<gpio_num_t>(GPIO_BL), 0 );
	}
...
#if 1
		const esp_err_t result = spi_device_transmit( SPIHandle, &SPITransaction );
#else
...
void lcdDrawFillRect(TFT_t *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
	...
	printf("_x1: %u; _x2: %u; _y1: %u; _y2: %u\n", _x1, _x2, _y1, _y2);
	const uint16_t size = _y2-_y1+1;
...
void DrawFillRect(TFT_t *dev, uint16_t topX, uint16_t topY, uint16_t width, uint16_t height, uint16_t color)
{
    if (topX + width > dev->_width || topY + height > dev->_height)
    {
        return;
    }

    uint16_t _x1 = topX + dev->_offsetx;
    uint16_t _x2 = _x1 + width - 1;
    uint16_t _y1 = topY + dev->_offsety;
	...
	for (uint16_t i = 0; i < fullCount; ++i)
	{
		...
		const auto ret = spi_device_transmit(dev->_SPIHandle, &spiTransaction);
		if (ret != ESP_OK)
		{
...
void DrawRectBuf(TFT_t *dev, uint16_t topX, uint16_t topY
	, uint16_t width, uint16_t height, uint16_t *pColors)
{
    if (topX + width > dev->_width || topY + height > dev->_height)
    {
        return;
	...
	spi_transaction_t spiTransaction;
	memset(&spiTransaction, 0, sizeof(spi_transaction_t));
	spiTransaction.tx_buffer = pColors;
	spiTransaction.length = byteCount * 8;
	const auto ret = spi_device_transmit(dev->_SPIHandle, &spiTransaction);
	if (ret != ESP_OK)
	{
		printf("error transmitting: %d", ret);
...
void lcdDrawLine(TFT_t *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint8_t width, uint16_t color)
{
	int i;
	int dx,dy;
	int sx,sy;
	...
	if ( dx > dy )
	{
		...
		for ( i = 0 ; i <= dx ; i++ )
		{
			lcdDrawPixel(dev, x1, y1, width, color);
			x1 += sx;
			E += 2 * dy;
			if ( E >= 0 )
			{
...
void DrawLine(TFT_t *dev, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
    /* distance between two points */
    ...
    if (dx > dy)
    {
        ...
        for (uint16_t i = 0; i <= dx; ++i)
        {
            ...
            if (E >= 0)
            {
                if (width > 0)
                {
                    DrawFillRect(dev, x1, y1-1, width, 2, color);
                }
                else
                {
                    DrawFillRect(dev, x1 + width, y1-1, -width, 2, color);
                }
                y1 += sy;
                x1 += width;
                width = 0;
        ...
        {
			if (width > 0)
            	DrawFillRect(dev, x1, y1, width, 1, color);
			else
				DrawFillRect(dev, x1 + width, y1, -width, 1, color);
        }
    }
    else // inclination >= 1
    {
        ...
        for (uint16_t i = 0; i <= dy; i++)
        {
            ...
            if (E >= 0)
            {
                if (height > 0)
                {
                    DrawFillRect(dev, x1, y1, 1, height, color);
                }
                else
                {
                    DrawFillRect(dev, x1, y1+height, 1, -height, color);
                }
                x1 += sx;
                y1 += height;
                height = 0;
...
void lcdBacklightOff(TFT_t *dev)
{
	if (dev->_bl >= 0)
	{
		gpio_set_level(static_cast<gpio_num_t>(dev->_bl), 0 );
	}
}
